...
class Embed_pad(nn.Module):
    ...
    def zero_pad(self, inputs, dim_in, dim_out):
        # About how to set paddings, refer to: https://pytorch.org/docs/stable/generated/torch.nn.functional.pad.html
    ...
    def forward(self, x):
        x = x.view(x.shape[0], -1) 
        x = self.zero_pad(x, self.dim_in, self.dim_out)
        x = self.l2norm(x)
        return x
