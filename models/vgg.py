...
class VGG(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False, flatGroupOut=False, kd2=False):
        ...
        block_out0 = [block.out for block in self.block0]
        block_out1 = [block.out for block in self.block1]
        block_out2 = [block.out for block in self.block2]
        block_out3 = [block.out for block in self.block3]
        block_out4 = [block.out for block in self.block4]
        ...
        if kd2:
            ...
            f4_temp = self.pool4(f4)
            f4_2 = f4_temp.view(f4_temp.size(0), -1)

        if is_feat:
            if kd2:
                # for 2 kds
                if preact:
                    return [f0, f1_pre, f2_pre, f3_pre, f4_pre, f5], [f0_2, f1_2, f2_2, f3_2, f4_2, f5], [block_out1, block_out2, block_out3, block_out4], x
                else:
                    return [f0, f1, f2, f3, f4, f5], [f0_2, f1_2, f2_2, f3_2, f4_2, f5], [block_out1, block_out2, block_out3, block_out4], x
            else:
                # for 1 kd
                if preact:
                    return [f0, f1_pre, f2_pre, f3_pre, f4_pre, f5], [block_out1, block_out2, block_out3, block_out4], x # org
                else:
