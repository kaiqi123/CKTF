...
def get_cifar100_dataloaders(batch_size=128, num_workers=8, is_instance=False, train_target_labels=[], test_target_labels=[]):
    """
    cifar 100
    """
    assert train_target_labels != []
    assert test_target_labels != []
...
class CIFAR100InstanceSample(datasets.CIFAR100):
    ...
    def __init__(self, root, train=True,
                 ...
                 download=False, k=4096, mode='exact', is_sample=True, percent=1.0, target_labels = []):
        super().__init__(root=root, train=train, download=download,
                         transform=transform, target_transform=target_transform, target_labels=target_labels)
        
        assert target_labels != [] # new add
...
def get_cifar100_dataloaders_sample(batch_size=128, num_workers=8, k=4096, mode='exact', is_sample=True, percent=1.0, train_target_labels=[], test_target_labels=[]):
    """
    cifar 100
    """
    assert train_target_labels != []
